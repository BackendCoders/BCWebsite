                    ...
                    <table class="min-w-full divide-y divide-slate-200/70 text-left">
                        ...
                        <tbody class="divide-y divide-slate-200/70 text-sm">
                            @forelse($recentApplications as $application)
                                <tr>
                                    <td class="py-4 pr-4 font-medium text-slate-900">{{ $contacts->first_name }}</td>
                                    <td class="py-4 pr-4 font-medium text-slate-900">{{ $contacts->last_name }}</td>
                                    <td class="py-4 pr-4 text-slate-500">{{ $contacts->phone ?? '-' }}</td>
                                    <td class="py-4 pr-4 text-slate-500">{{ $contacts->email }}</td>
                                    <td class="py-4 pr-4 text-slate-500">{{ $contacts->message }}</td>
