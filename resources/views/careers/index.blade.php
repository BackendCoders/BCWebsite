...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-3xl font-bold text-slate-900">Career openings</h2>
            <p class="mt-1 text-sm text-slate-500">Manage open positions from the backend.</p>
        </div>
                ...
                <tbody class="divide-y divide-slate-200/70">
                    @forelse($careers as $career)
                        <tr class="hover:bg-slate-50/70">
                            ...
                            <td class="px-6 py-4 text-slate-600">{{ $career->employment_type ?? '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $career->work_mode ?? '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $career->experience ?? '-' }}</td>
                            <td class="px-6 py-4">
                                @if($career->is_active)
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-slate-500">No career openings found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
