...
<div class="mx-auto max-w-4xl space-y-6">
    <div>
        <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Add blog</h2>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Create a clean backend blog entry.</p>
    </div>
        ...
        <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data" class="grid gap-5">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10 dark:border-white/10 dark:bg-slate-950/40 dark:text-white">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-white/10 dark:bg-slate-950/40">
                    <input id="is_published" type="checkbox" name="is_published" value="1" class="h-5 w-5 rounded border-slate-300 text-[#FD5528] focus:ring-[#FD5528]" @checked(old('is_published'))>
            ...
            <div>
                ...
                <textarea name="excerpt" rows="3" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10 dark:border-white/10 dark:bg-slate-950/40 dark:text-white">{{ old('excerpt') }}</textarea>
            </div>

            <div>
                <label class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200">Content</label>
                <textarea name="content" rows="8" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10 dark:border-white/10 dark:bg-slate-950/40 dark:text-white">{{ old('content') }}</textarea>
            </div>
