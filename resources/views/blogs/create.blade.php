...
<div class="relative mx-auto mt-32 max-w-5xl space-y-6 overflow-hidden">
    ...
    <div class="flex flex-col gap-4 rounded-[2.25rem] border border-slate-200/70 bg-white/75 p-6 shadow-2xl shadow-slate-200/40 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
        <div class="space-y-2">
            ...
            <div>
                <h2 class="text-3xl font-bold tracking-tight text-slate-900">Add blog</h2>
                <p class="mt-1 max-w-2xl text-sm text-slate-500">Create a clean backend blog entry with a polished orange-white layout.</p>
            </div>
        </div>
        ...
        <div class="rounded-[2rem] border border-slate-200/70 bg-white/85 p-6 shadow-xl shadow-slate-200/40 backdrop-blur-xl">
            <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data" class="grid gap-5 pb-24" data-blog-form>
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="3" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10">{{ old('excerpt') }}</textarea>
                </div>
