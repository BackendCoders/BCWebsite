...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-3xl font-bold text-slate-900">Blog posts</h2>
            <p class="mt-1 text-sm text-slate-500">Manage blog content from the backend.</p>
        </div>
                ...
                <tbody class="divide-y divide-slate-200/70">
                    @forelse($blogs as $blog)
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-6 py-4">
                                @if($blog->image)
                                    <img src="{{ asset('storage/' . $blog->image) }}" alt="{{ $blog->title }}" class="h-14 w-20 rounded-2xl object-cover ring-1 ring-slate-200">
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-slate-500">No blog posts found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
