...
<div class="relative mx-auto mt-32 max-w-5xl space-y-6 overflow-hidden">
    ...
    <div class="flex flex-col gap-4 rounded-[2.25rem] border border-slate-200/70 bg-white/75 p-6 shadow-2xl shadow-slate-200/40 backdrop-blur-xl lg:flex-row lg:items-center lg:justify-between">
        <div class="space-y-2">
            ...
            <div>
                ...
                <p class="mt-1 max-w-2xl text-sm text-slate-500">Update category details with a more polished orange-white admin layout.</p>
            </div>
        </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
        <div class="rounded-[2rem] border border-slate-200/70 bg-white/85 p-6 shadow-xl shadow-slate-200/40 backdrop-blur-xl">
            <form action="{{ route('categories.update', $category->id) }}" method="POST" class="grid gap-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ $category->name }}" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-sm font-medium text-slate-700">Slug</label>
                        <input type="text" name="slug" value="{{ $category->slug }}" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Description</label>
                    <textarea name="description" rows="5" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-[#FD5528] focus:ring-4 focus:ring-[#FD5528]/10">{{ $category->description }}</textarea>
                </div>
                ...
                <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-[#FD5528] px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-[#FD5528]/25 transition hover:bg-[#e94c20] hover:shadow-xl hover:shadow-[#FD5528]/30 active:scale-[0.98]">Update Category</button>
                </div>
            </form>
        </div>
        ...
        <div class="space-y-4">
            <div class="rounded-[2rem] border border-slate-200/70 bg-white/85 p-6 shadow-xl shadow-slate-200/40 backdrop-blur-xl">
                ...
                <h3 class="mt-2 text-xl font-bold text-slate-900">Edit with clarity</h3>
                <ul class="mt-4 space-y-3 text-sm leading-6 text-slate-500">
                    <li class="flex gap-3">
                        <span class="mt-1 h-2 w-2 rounded-full bg-[#FD5528]"></span>
                        Keep the category name easy to scan in lists and tables.
                    </li>
                    <li class="flex gap-3">
                        <span class="mt-1 h-2 w-2 rounded-full bg-[#FD5528]"></span>
                        Make the slug simple, lowercase, and descriptive.
                    </li>
                    <li class="flex gap-3">
                        <span class="mt-1 h-2 w-2 rounded-full bg-[#FD5528]"></span>
                        Short descriptions keep the admin UI feeling neat.
                    </li>
                </ul>
            </div>
            ...
            <div class="rounded-[2rem] border border-slate-200/70 bg-gradient-to-br from-[#FD5528] to-[#e94c20] p-6 text-white shadow-xl shadow-[#FD5528]/25">
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-white/70">Theme</p>
                <h3 class="mt-2 text-xl font-bold">Orange and white admin style</h3>
                <p class="mt-3 text-sm leading-6 text-white/85">This keeps the edit experience aligned with the rest of the dashboard while making it feel more premium.</p>
            </div>
        </div>
    </div>
</div>
